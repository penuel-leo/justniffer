...
void parser::process_opening_connection(tcp_stream *ts, struct timeval* t, unsigned char* packet)
{
	...
	if (it == connections.end())
	{
        ...
        if (connections.size() < 2048) {//如果map已经大于2048了，则不再新增，除非其中一个stream处理完成erase了，导致有些stream永存在里面其他进不来?
            stream::ptr pstream(new stream(this, factories.begin(), factories.end(), _printer, this));
            pstream->onOpening( ts, t);
        ...
        if (_discard_counter > 2048) {
            _discard_counter = 0;//当抛弃的stream数 > 2048清空connections
            connections.clear();
        }
	}	
}
